...
def get_costcenterwisestockissue(item_name, startDate, endDate, cursor, outlet):
    ...
    params = [item_name, outlet]

    # ✅ Handle empty dates → latest 20
    if startDate == "" and endDate == "":
        query = """
            SELECT r.CostCenter, d.ItemName, d.GroupName, d.BrandName,
                   d.Amount, d.UOM, d.Rate,
                   (d.Amount * d.Rate) AS Total,
                   r.Date, d.idintblStoreReqDetails, d.StoreReqID
    ...
    for row in rows:
        entry = {
            ...
            "Total": float(row[7]),
            "Date": str(row[8]),
            "idintblStoreReqDetails" : str(row[9]),
            "StoreReqID" : str(row[10]),
