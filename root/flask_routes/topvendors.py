...
@app_file51.route('/topvendors', methods = ["POST"])
@cross_origin()
def get_topvendors():
    try:
        ...
        sql_query = """select Company_Name, sum(TotalAmount) as TotalAmount , sum(TotalAmount - TaxAmount) as NetAmount from intbl_purchaserequisition group by Company_Name order by TotalAmount DESC """ 
