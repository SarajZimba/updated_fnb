...
def stats():
    try:
        ...
        prev_date = (input_date - timedelta(days=1)).strftime("%Y-%m-%d")
        input_date = input_date.strftime("%Y-%m-%d")

        # -------- TRY ORIGINAL DATE --------
        sql = """
            SELECT idtblorderHistory as order_id
            FROM tblorderhistory
            WHERE Outlet_OrderID=%s AND Date=%s AND Outlet_Name=%s
        """
        cursor.execute(sql, (data["outlet_orderID"], input_date, data["Outlet_Name"]))
        result = cursor.fetchall()

        # -------- IF NOT FOUND, TRY PREVIOUS DATE --------
        if not result:
            cursor.execute(sql, (data["outlet_orderID"], prev_date, data["Outlet_Name"]))
            result = cursor.fetchall()
            used_date = prev_date
        else:
            used_date = input_date

        # -------- STILL NOT FOUND --------
        if not result:
            return {"error": "No data found for given date or previous day."}, 400

        # -------- DELETE --------
        row_headers = [x[0] for x in cursor.description]
        ids = [dict(zip(row_headers, r)) for r in result]

        for item in ids:
            pk = item["order_id"]

            cursor.execute(
                "DELETE FROM tblorder_detailshistory WHERE order_ID=%s",
                (pk,)
            )
            cursor.execute(
                "DELETE FROM tblorderhistory WHERE idtblorderHistory=%s",
                (pk,)
            )

        # Delete from CreditHistory using the actual matched date
        cursor.execute("""
            DELETE FROM CreditHistory
            WHERE Outlet_OrderID=%s AND DATE(Date)=%s AND outetName=%s
        """, (data["outlet_orderID"], used_date, data["Outlet_Name"]))

        mydb.commit()
        mydb.close()

        return {"success": f"Deleted data for date {used_date}"}, 200
